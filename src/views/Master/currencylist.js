...
export default function CurrencyList() {
  ...
  useEffect(() => {        
    if(deleteData){
        refetch()
        successMessage(deleteData.delete_currency_by_pk.id)
    }
    if(deleteError){
      ...
      if(deleteError.graphQLErrors[0].message.includes("Foreign key violation")){        
        errorMessage("Delete Failed !. This Currency is used in Invoice created.")
      }
      else{
  ...
  const columnSet = [
    ...
    renderCell: (params) => {
        return (
          <IconButton color="error" aria-label="Delete Currency" 
            onClick={() => {
              deleteItem({
                variables: {
                  id: params.row.id
